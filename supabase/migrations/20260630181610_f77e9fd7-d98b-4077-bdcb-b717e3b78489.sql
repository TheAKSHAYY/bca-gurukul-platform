  ...
  SELECT * FROM (
    ...
    SELECT
      ...
      (
        ...
        CASE WHEN c.code ILIKE safe_query.raw OR c.slug ILIKE safe_query.raw THEN 80 ELSE 0 END +
        CASE WHEN c.description ILIKE safe_query.like_pattern OR c.code ILIKE safe_query.like_pattern OR c.slug ILIKE safe_query.like_pattern THEN 10 ELSE 0 END
      ) AS rank
    ...
    WHERE c.status = 'published'
      ...
      AND (
        ...
        OR c.code ILIKE safe_query.like_pattern
        OR c.slug ILIKE safe_query.like_pattern
    ...
    SELECT
      ...
      (
        ...
        CASE WHEN sem.title ILIKE safe_query.raw THEN 100 ELSE 0 END +
        CASE WHEN sem.title ILIKE safe_query.raw || '%' THEN 50 ELSE 0 END +
        CASE WHEN sem.description ILIKE safe_query.like_pattern OR sem.number::text ILIKE safe_query.raw THEN 10 ELSE 0 END
      )
    FROM public.semesters sem, safe_query
    WHERE sem.status = 'published'
      AND sem.deleted_at IS NULL
      AND (
        sem.title ILIKE safe_query.like_pattern
        OR sem.description ILIKE safe_query.like_pattern
        OR sem.number::text ILIKE safe_query.like_pattern
        OR (setweight(to_tsvector('english', COALESCE(sem.title, '')), 'A') ||
            setweight(to_tsvector('english', COALESCE(sem.description, '')), 'B')) @@ safe_query.tsq
      )
    ...
    SELECT
      ...
      (
        ...
        CASE WHEN s.title ILIKE safe_query.raw THEN 100 ELSE 0 END +
        CASE WHEN s.title ILIKE safe_query.raw || '%' THEN 50 ELSE 0 END +
        CASE WHEN s.code ILIKE safe_query.raw OR s.slug ILIKE safe_query.raw THEN 80 ELSE 0 END +
        CASE WHEN s.description ILIKE safe_query.like_pattern OR s.code ILIKE safe_query.like_pattern OR s.slug ILIKE safe_query.like_pattern THEN 10 ELSE 0 END
      )
    FROM public.subjects s, safe_query
    WHERE s.status = 'published'
      AND s.deleted_at IS NULL
      AND (
        s.title ILIKE safe_query.like_pattern
        OR s.description ILIKE safe_query.like_pattern
        OR s.code ILIKE safe_query.like_pattern
        OR s.slug ILIKE safe_query.like_pattern
        OR (setweight(to_tsvector('english', COALESCE(s.title, '')), 'A') ||
            setweight(to_tsvector('english', COALESCE(s.description, '')), 'B')) @@ safe_query.tsq
      )
    ...
    SELECT
      ...
      (
        ...
        CASE WHEN n.slug ILIKE safe_query.raw THEN 60 ELSE 0 END +
        CASE WHEN n.summary ILIKE safe_query.like_pattern OR n.body ILIKE safe_query.like_pattern OR n.slug ILIKE safe_query.like_pattern THEN 10 ELSE 0 END
      )
    ...
    WHERE n.status = 'published'
      ...
      AND (
        ...
        OR n.slug ILIKE safe_query.like_pattern
    ...
    SELECT
      ...
      (
        ...
        CASE WHEN p.exam_type ILIKE safe_query.raw OR p.paper_number ILIKE safe_query.raw THEN 40 ELSE 0 END +
        CASE WHEN p.description ILIKE safe_query.like_pattern OR p.exam_type ILIKE safe_query.like_pattern OR p.paper_number ILIKE safe_query.like_pattern THEN 10 ELSE 0 END
      )
    ...
    WHERE p.status = 'published'
      ...
      AND (
        ...
        OR p.exam_type::text ILIKE safe_query.like_pattern
        OR p.paper_number::text ILIKE safe_query.like_pattern
    ...
    SELECT
      ...
      (
        ...
        CASE WHEN z.slug ILIKE safe_query.raw THEN 60 ELSE 0 END +
        CASE WHEN z.description ILIKE safe_query.like_pattern OR z.instructions ILIKE safe_query.like_pattern OR z.slug ILIKE safe_query.like_pattern THEN 10 ELSE 0 END
      )
    ...
    WHERE z.status = 'published'
      ...
      AND (
        ...
        OR z.slug ILIKE safe_query.like_pattern
