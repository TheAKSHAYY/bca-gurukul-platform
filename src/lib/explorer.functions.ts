...
export const updateExplorerNode = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const table = type === "course" ? "courses"
      : type === "semester" ? "semesters"
      : type === "subject" ? "subjects" : "units";

    const payload: Record<string, unknown> = {};
    if (patch.name != null) payload[titleField] = patch.name;
...
export const deleteExplorerNode = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    const table = data.type === "course" ? "courses"
      : data.type === "semester" ? "semesters"
      : data.type === "subject" ? "subjects" : "units";
    const { error } = await context.supabase.from(table)
...
export const duplicateExplorerNode = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    if (data.type === "semester" || data.type === "unit") {
      ...
      const { data: next } = await sb.from(table)
        ...
        .order("number", { ascending: false }).limit(1);
      copy.number = ((next?.[0]?.number as number) ?? 0) + 1;
    }
    if (data.type === "course" || data.type === "subject") {
      const parentField = data.type === "course" ? null : "semester_id";
      const filter = sb.from(table).select("sort_order");
      if (parentField) filter.eq(parentField, src[parentField]);
      const { data: next } = await filter.is("deleted_at", null)
        .order("sort_order", { ascending: false }).limit(1);
      copy.sort_order = ((next?.[0]?.sort_order as number) ?? 0) + 1;
    }
...
export const reorderExplorerNode = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const n = neighbor?.[0] as { id: string;[k: string]: unknown } | undefined;
    if (!n) return { ok: true };
