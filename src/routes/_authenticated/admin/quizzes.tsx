      ...
      <main className="mx-auto max-w-6xl px-6 py-10">
        ...
        <section className="mt-8 grid gap-4 rounded-2xl border border-border bg-surface p-5 sm:grid-cols-4">
          ...
          <Picker
            ...
            disabled={!subjectId}
            onChange={setUnitId}
            options={(unitsQ.data ?? []).map((u) => ({
              value: u.id,
              label: `U${u.number} · ${u.title}`,
            }))}
          />
        </section>
                ...
                >
                  <div>
                    <div className="flex items-center gap-2">
                      ...
                      {q.time_limit_minutes && (
                        <Badge variant="outline">{q.time_limit_minutes}m</Badge>
                      )}
                    </div>
                    {q.description && (
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Button
                      ...
                      size="sm"
                      onClick={() => {
                        if (confirm(`Delete "${q.title}"?`)) del.mutate(q.id);
                      }}
                    >
